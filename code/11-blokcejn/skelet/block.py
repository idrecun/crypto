...
def valid_pow(blk, bits=DIFFICULTY_BITS):
    # TODO (vežbe): blok zadovoljava dokaz rada ako je heš zaglavlja, shvaćen kao
    # ceo broj, manji od ciljne vrednosti target(bits) (tj. ima ~bits vodećih nula).
    raise NotImplementedError("valid_pow: proveriti da je block_hash(blk) < target(bits)")


def mine(blk, bits=DIFFICULTY_BITS):
    # TODO (vežbe): povećavaj blk["nonce"] dok valid_pow(blk, bits) ne postane tačno.
    raise NotImplementedError("mine: tražiti nonce koji zadovoljava valid_pow")
