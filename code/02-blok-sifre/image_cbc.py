...
def main():
    ...
    ap.add_argument(
        ...
        help="Demo key (will be padded/truncated to 16 bytes)",
    )
    ap.add_argument(
        "--decrypt",
        action="store_true",
        help="Decrypt instead of encrypt (for sanity check)",
    )
    args = ap.parse_args()
    ...
    raw = (
        img.tobytes()
    )  # raw pixel bytes: repeating patterns in image => repeating blocks in ECB
    block_size = 16
